        ...
        <div class="modal-dialog modal-dialog-scrollable modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('intranatal_care.store_ropb') }}" method="post" autocomplete="off">
                        @csrf
                        <div class="row">
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#ropb-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: '{{ route('intranatal_care.data_ropb') }}',
                scrollX: true,
                fixedHeader: true,
                columns: [{
                        data: 'NIK',
                    ...
                    },
                    {
                        ...
                        searchable: false,
                        render: function(data, type, row) {
                            let viewUrl = '{{ route('intranatal_care.show_ropb', ':id') }}'
                                .replace(':id', row.id);
                            let editUrl = '{{ route('intranatal_care.edit_ropb', ':id') }}'
                                .replace(':id', row.id);
                            let deleteUrl = '{{ route('intranatal_care.destroy_ropb', ':id') }}'
                                .replace(':id', row.id);
                            return `
        ...
        $('#ropb-table').on('click', '.view-btn', function() {
            let id = $(this).data('id');
            $.ajax({
                url: '{{ route('intranatal_care.show_ropb', ':id') }}'.replace(':id', id),
                method: 'GET',
        ...
        $('#ropb-table').on('click', '.edit-btn', function() {
            let id = $(this).data('id');
            $.ajax({
                url: '{{ route('intranatal_care.edit_ropb', ':id') }}'.replace(':id', id),
                method: 'GET',
                success: function(data) {
                    ...
                    $('#editForm').attr('action', '{{ route('intranatal_care.update_ropb', ':id') }}'
                        .replace(
                            ':id', id));
